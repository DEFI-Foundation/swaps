...
contract WBTCUSDTSwap is MetalSwapAbstract  {
    ...
    function createSwap (uint256 period, uint256 targetSize, uint16 coverX100, SwapType swapType) public payable override nonReentrant checkSCUnpaused() returns (uint256 swapID) {

        require (period>= minTimeSwap,"Error on createSwap: cannot create the requested swap because period of the swap is to short");
        require (verifySwapPermission(targetSize, swapType), "Error on createSwap: cannot create the requested swap because out of safety margins!");
        ...
        if(swapType==SwapType(0)){          //swapType==SwapType(0) is the swapType WBTC->usd
            threshold = currentPrice+currentPrice*(coverX100-safetyMarginX100)/100;
            ERC20(erc20Asset).safeTransferFrom(msg.sender, address(premiumAddress), formatAmount(periodFee, 18, assetDecimals));
            ERC20(erc20Asset).safeTransferFrom(msg.sender, address(settlementFeeContainer), formatAmount(settlementFee, 18, assetDecimals));
    ...
    function closeSwap (uint256 swapID) public override nonReentrant checkSCUnpaused() {
        ...
        if (swap.swapType == SwapType(0)) {
            settlementFeeContainer.payLiquidationFee(payable(msg.sender), formatAmount(settlementFee, 18, assetDecimals), address(erc20Asset));
            emit ClosedSwap (swapID, swap.holder, formatAmount(profit, 18, currencyDecimals), formatAmount(remainingCoverReturn, 18, assetDecimals), block.timestamp);
        }
        else {
            ...
            emit ClosedSwap (swapID, swap.holder, formatAmount(profit, 18, assetDecimals), formatAmount(remainingCoverReturn, 18, currencyDecimals), block.timestamp);
        } 
    }

    function executeSwap (uint256 swapID) public override nonReentrant checkSCUnpaused() {
        Swap storage swap = swaps[swapID];
        ...
        if(swap.swapType==SwapType(0)) {
            settlementFeeContainer.payLiquidationFee(payable(msg.sender), formatAmount(settlementFee, 18, assetDecimals), address(erc20Asset));
            emit ExecutedSwap (swapID, swap.holder, msg.sender, formatAmount(profit, 18, currencyDecimals), formatAmount(remainingCoverReturn, 18, assetDecimals));
        }
        else {
            settlementFeeContainer.payLiquidationFee(payable(msg.sender), formatAmount(settlementFee, 18, currencyDecimals), address(erc20Currency));
            emit ExecutedSwap (swapID, swap.holder, msg.sender, formatAmount(profit, 18, assetDecimals), formatAmount(remainingCoverReturn, 18, currencyDecimals));
        }
    }

    function marginCall (uint256 swapID) public override nonReentrant checkSCUnpaused() {
        Swap storage swap = swaps[swapID];
        ...
        if (swap.swapType==SwapType(0)) {
            ...
            settlementFeeContainer.payLiquidationFee(payable(msg.sender), formatAmount(FeeManager(feeManager).getSettlementFee(swap.targetSize), 18, assetDecimals), address(erc20Asset));
        }
        else {
            swapAvgCurrency -= swap.targetSize;
            settlementFeeContainer.payLiquidationFee(payable(msg.sender), formatAmount(FeeManager(feeManager).getSettlementFee(swap.targetSize), 18, currencyDecimals), address(erc20Currency));
        }
        
        emit LiquidatedSwap (swapID, swap.holder, msg.sender, block.timestamp);
    }

    function addCover (uint256 swapID, uint16 newCoverX100) public payable override nonReentrant checkSCUnpaused() {
        ...
        if ((swap.swapType == SwapType(0))) {       //swapType==SwapType(0) is the swapType ETH->usd
            ...
            erc20Asset.safeTransferFrom(swap.holder, address(poolAsset), formatAmount(coverToAdd, 18, assetDecimals));
            poolAsset.lock(formatAmount(coverToAdd, 18, assetDecimals));
    ...
    function payProfit (Swap memory swap, uint256 penalties, uint256 currentPrice,uint256 rewardAmount) internal override returns (uint256 profit, uint256 remainingCoverReturn){
        ...
        if (swap.swapType == SwapType(0)) {             //swapType==SwapType(0) is the swapType ETH->usd
            ...
            if (priceDifference>=0) {
                ...
                if (remainingCoverPenalities > 0) {
                    poolAsset.send(swap.holder, formatAmount(uint(remainingCoverPenalities), 18, assetDecimals));
                    unlockFunds(swap.swapType, cover);
                ...
                poolAsset.send(swap.holder, formatAmount(cover - penalties, 18, assetDecimals));
                unlockFunds(swap.swapType, cover);
        ...
        if(swap.swapType==SwapType(1)){             //swapType==SwapType(1) is the swapType USD->eth
            ...
            if (priceDifference <= 0) {
                ...
                poolAsset.sendProfit(swap.holder, formatAmount(profit, 18, assetDecimals));
    ...
    function lockFunds (SwapType _swapType, uint256 _cover, address payable _swapHolder) internal override  {
        if(_swapType==SwapType(0)){
            ERC20(erc20Asset).safeTransferFrom(_swapHolder, address(poolAsset), formatAmount(_cover, 18, assetDecimals));
            poolAsset.lock(formatAmount(_cover, 18, assetDecimals));
    ...
    function unlockFunds (SwapType _swapType, uint256 _cover) internal override  {
        if (_swapType==SwapType(0)) {
            poolAsset.unlock(formatAmount(_cover, 18, assetDecimals));
        } else {
    ...
    function setOperatingAllowance () override public onlyGovernance {
        ...
        erc20Asset.safeApprove(address(poolAsset), type(uint256).max);
        erc20Asset.safeApprove(address(settlementFeeContainer), type(uint256).max);
        erc20Asset.safeApprove(address(premiumAddress), type(uint256).max);
    }
