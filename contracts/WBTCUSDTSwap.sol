...
contract WBTCUSDTSwap is MetalSwapAbstract {
        ...
        returns (uint256 swapID)
    {
        ...
        if (swapType == SwapType(0)) {
            //swapType==SwapType(0) is the swapType WBTC->usd
            threshold =
                currentPrice +
                (currentPrice * (coverX100 - safetyMarginX100)) /
                100;
            ERC20(erc20Asset).safeTransferFrom(
                msg.sender,
                address(premiumAddress),
                formatAmount(periodFee, 18, assetDecimals)
            );
            ERC20(erc20Asset).safeTransferFrom(
                msg.sender,
                address(settlementFeeContainer),
                formatAmount(settlementFee, 18, assetDecimals)
            );
            swapAvgAsset += (targetSize * currentPrice) / PRICE_DECIMALS; //update of the avgSwap value
        } else {
        ...
        checkSCUnpaused
    {
        ...
        if (swap.swapType == SwapType(0)) {
            ...
            emit ClosedSwap(
                ...
                formatAmount(remainingCoverReturn, 18, assetDecimals),
                block.timestamp,
                currentPrice
            );
        } else {
            ...
            emit ClosedSwap(
                swapID,
                swap.holder,
                formatAmount(profit, 18, assetDecimals),
        ...
        checkSCUnpaused
    {
        ...
        if (swap.swapType == SwapType(0)) {
            ...
            settlementFeeContainer.payLiquidationFee(
                payable(msg.sender),
                formatAmount(
                    FinancialSwapManager(financialSwapManager).getSettlementFee(
                        swap.targetSize
                    ),
                    18,
                    assetDecimals
        ...
        checkSCUnpaused
    {
        ...
        if ((swap.swapType == SwapType(0))) {
            ...
            erc20Asset.safeTransferFrom(
                swap.holder,
                address(poolAsset),
                formatAmount(coverToAdd, 18, assetDecimals)
            );
    ...
    function payProfit(
        ...
        if (swap.swapType == SwapType(0)) {
            ...
            if (priceDifference >= 0) {
                ...
                poolAsset.send(
                    swap.holder,
                    formatAmount(cover - penalties, 18, assetDecimals)
                );
                unlockFunds(swap.swapType, cover);
    ...
    function lockFunds(
        ...
        if (_swapType == SwapType(0)) {
            ERC20(erc20Asset).safeTransferFrom(
                _swapHolder,
                address(poolAsset),
                formatAmount(_cover, 18, assetDecimals)
            );
