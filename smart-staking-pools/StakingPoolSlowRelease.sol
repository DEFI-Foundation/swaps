...
contract StakingPoolSlowRelease is StakingPoolV2base {
    ...
    uint256 public periodLockupPercent; //range [0-100*1M]=[0-100000000], 0,555555% each day lasts for 180 days until complete unlock
    ...
    event FinalizeRedeemParameters(
        uint256 _periodLockupPercent,
        uint256 _lockupPeriodInSec,
        bool finalized,
        bool paused
    );
    ...
    function finalizeRedeemParameters(
        uint256 _periodLockupPercent,
        uint256 _lockupPeriodInSec
    ) external onlyGovernance nonReentrant {
        require(
            !finalized,
            'Error: Staking Pool cannot already be finalized in order to finalize redeem parameters!'
        );
        periodLockupPercent = _periodLockupPercent;
        lockupPeriod = _lockupPeriodInSec;
        finalized = true;
        paused = false;
        emit FinalizeRedeemParameters(
            _periodLockupPercent,
            _lockupPeriodInSec,
            finalized,
            paused
        );
        ...
        checkStakingUnpaused
    {
        require(
            ...
            'Error: user has not staked'
        );
        require(
            block.timestamp >=
                (userRedeemData[msg.sender].lastRedeemTimestamp + lockupPeriod),
            'Error: not enough time has passed from the last call to receiveNewTokens'
        );
        ...
        if (!userRedeemData[msg.sender].hasBegunRedeem) {
            ...
            userRedeemData[msg.sender].leftOverToRedeem =
                tokensStakedPerUser[msg.sender] +
                calcReward(msg.sender);
        }

        uint256 tokensToRedeem = ((tokensStakedPerUser[msg.sender] +
            calcReward(msg.sender)) *
            periodLockupPercent *
            (block.timestamp -
                userRedeemData[msg.sender].lastRedeemTimestamp)) /
            (mult * 100 * lockupPeriod);
        ...
        require(
            tokensToRedeem > 0,
            'Error: you have not matured any tokens to redeem yet'
        );
        userRedeemData[msg.sender].lastRedeemTimestamp =
            block.timestamp -
            ((block.timestamp -
                userRedeemData[msg.sender].lastRedeemTimestamp) % lockupPeriod);
        require(
            stakingToken.transfer(msg.sender, tokensToRedeem),
    ...
    function getRedeemUserData(address user)
        external
        view
        returns (
            bool hasBegunRedeem,
    ...
    function calcCurrentlyMaturedReward(address user)
        external
        view
        returns (uint256 tokensToRedeem)
    {
        if (block.timestamp >= endStaking) {
            if (userRedeemData[user].hasBegunRedeem) {
                tokensToRedeem =
                    ((tokensStakedPerUser[user] + calcReward(user)) *
                        periodLockupPercent *
                        (block.timestamp -
                            userRedeemData[user].lastRedeemTimestamp)) /
                    (mult * 100 * lockupPeriod);
                ...
                if (
                    ...
                    tokensToRedeem = (tokensStakedPerUser[user] +
                        calcReward(user));
                }
            }
